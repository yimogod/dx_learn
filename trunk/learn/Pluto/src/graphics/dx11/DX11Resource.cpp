...
DX11Resource::DX11Resource(GraphicsResource* gtResource)
...
GraphicsResource* DX11Resource::GetResource(){
    return static_cast<GraphicsResource*>(_pluObj);
}

ID3D11Resource* DX11Resource::GetDXResource(){
    return static_cast<ID3D11Resource*>(_dxObj);
}

ID3D11Resource* DX11Resource::GetStagingResource(){
    return mStaging;
}

D3D11_MAPPED_SUBRESOURCE DX11Resource::MapForWrite(ID3D11DeviceContext* context, int sri){
...
void DX11Resource::Unmap(ID3D11DeviceContext* context, int sri){
    context->Unmap(GetDXResource(), sri);
}

bool DX11Resource::PreparedForCopy(D3D11_CPU_ACCESS_FLAG access){
...
int const DX11Resource::msStagingAccess[] = {
